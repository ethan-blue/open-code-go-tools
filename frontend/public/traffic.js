  ...
  if (show) {
    loadingTimer = setTimeout(() => {
      ...
      if (row && !row.hasChildNodes()) {
        row.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-3);">' + tdT('td_proxy_offline', 'Proxy not responding. Please check the proxy service.') + '</div>';
      }
    }, 10000);
  }
}
...
function initTimeRange() {
  ...
  if (!container) return;
  const days = [
    { label: tdT('sessions_period_today', 'Today'), value: 1 },
    { label: '7' + tdT('sessions_period_month', 'd').replace(/^./, ''), value: 7 },
    { label: '30' + tdT('sessions_period_month', 'd').replace(/^./, ''), value: 30 },
    { label: tdT('sessions_period_all', 'All'), value: 365 }
  ];
  // Simpler approach: just use fixed labels with tdT
  const labels = [
    { label: tdT('td_today', 'Today'), value: 1 },
    { label: tdT('td_7d', '7d'), value: 7 },
    { label: tdT('td_30d', '30d'), value: 30 },
    { label: tdT('sessions_period_all', 'All'), value: 365 }
  ];
  container.innerHTML = labels.map(d =>
...
function updateTimestamp() {
  ...
  if (el) el.textContent = tdT('td_updated', 'Updated') + ' ' + new Date().toLocaleTimeString();
}
...
function renderTopStats(summary) {
  const lblTotal = tdT('traf_total', 'Total Requests');
  const lblRate = tdT('traf_rate', 'Success Rate');
  const lblLatency = tdT('traf_latency', 'Avg Latency');
  const lblTokens = tdT('traf_tokens', 'Token Usage');
  const failCount = (summary?.total_requests || 0) - (summary?.success_count || 0);
  const cards = [
    { label: lblTotal, val: fmtNum(summary?.total_requests || 0), color: DOPAMINE[0], sub: '' },
    { label: lblRate, val: (summary?.success_rate || 0).toFixed(1) + '%', color: DOPAMINE[1], sub: failCount + ' fail' },
    { label: lblLatency, val: (summary?.avg_latency_ms || 0).toFixed(0) + 'ms', color: DOPAMINE[2], sub: 'avg' },
    { label: lblTokens, val: fmtToken(summary?.total_tokens || 0), color: DOPAMINE[3], sub: '≈ ' + fmtCost(summary?.estimated_cost) },
    ...
    { label: 'Cache Hit', val: ((summary?.cache_hit_rate || 0)).toFixed(1) + '%', color: DOPAMINE[7], sub: (summary?.total_input_tokens || 0) > 0 ? fmtToken(summary?.total_cache_read_tokens || 0) + '/' + fmtToken(summary?.total_input_tokens) : '' },
  ];
...
function renderTokenTrend(daily, granularity) {
  ...
  if (!daily || daily.length === 0) {
    canvas.parentElement.innerHTML = '<div class="td-empty">' + tdT('td_no_data', 'No data') + '</div>'; return;
  }
...
function renderRequestTrend(daily, granularity) {
  ...
  chartInstances.requestTrend = new Chart(canvas, {
    type: 'line',
    data: { labels: daily.map(d => fmtTrendLabel(d.date, granularity)), datasets: [{ label: tdT('traf_total', 'Requests'), data: daily.map(d => d.requests), borderColor: DOPAMINE[7], backgroundColor: DOPAMINE[7] + '20', fill: true, tension: 0.4, cubicInterpolationMode: 'monotone', pointRadius: 3, pointHoverRadius: 6, yAxisID: 'y' }] },
...
function renderModelTable(models) {
  ...
  if (!container) return;
  if (!models || models.length === 0) {
    container.innerHTML = '<tr><td colspan="8" style="text-align:center;color:var(--text-3);padding:24px;">' + tdT('td_no_data', 'No data') + '</td></tr>'; return;
  }
  ...
  container.innerHTML = models.map((m, i) => {
    ...
    <td class="fwb">${tdT('td_total', 'Total')}</td>
...
function renderClientTable(clients) {
  ...
  if (!clients || clients.length === 0) {
    container.innerHTML = '<tr><td colspan="3" style="text-align:center;color:var(--text-3);padding:16px;">' + tdT('td_no_data', 'No data') + '</td></tr>'; return;
  }
...
function initTrafficDashboard() {
  ...
  // Only refresh if the traffic view is currently active
  ...
  window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', () => {
    applyChartTheme();
    // Rebuild charts only if view is active
  ...
  // 轮询刷新统计数据 — only when view is active
  setInterval(() => {
    const hv = document.getElementById('view-history');
    if (hv && hv.classList.contains('active')) {
      refreshAll();
...
/** Public API: call from app.js when switching to the traffic view */
function refreshTrafficDashboard() {
  refreshAll();
}
...
async function loadDetailData() {
  ...
  if (!tbody) return;
  tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;color:var(--text-3);padding:24px;"><span class="td-spinner"></span> ' + tdT('sessions_loading', 'Loading...') + '</td></tr>';

  const base = getBaseURL();
  const data = await apiGet(`${base}/ocgt/api/history?days=${_detailState.days}`);

  if (data && data._error) {
    tbody.innerHTML = `<tr><td colspan="10" style="text-align:center;color:var(--red);padding:24px;">${tdT('td_load_failed', 'Load failed')}: ${data._error}</td></tr>`;
    return;
  }
  ...
  if (ts) ts.textContent = tdT('td_updated', 'Updated') + ' ' + new Date().toLocaleTimeString();
}
...
function updateModelFilterOptions() {
  ...
  sel.innerHTML = '<option value="">' + tdT('sessions_filter_all', 'All Models') + '</option>' + models.map(m =>
...
function renderDetailTable() {
  ...
  if (filtered.length === 0) {
    tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;color:var(--text-3);padding:24px;">' + tdT('td_no_match', 'No matching records') + '</td></tr>';
...
function renderDetailPagination(totalPages) {
  ...
  let html = '<span class="pg-btn" data-page="' + (page - 1) + '" style="' + (page <= 1 ? 'opacity:0.3;pointer-events:none;' : '') + '">‹ ' + tdT('td_prev', 'Prev') + '</span>';
  ...
  html += '<span class="pg-btn" data-page="' + (page + 1) + '" style="' + (page >= totalPages ? 'opacity:0.3;pointer-events:none;' : '') + '">' + tdT('td_next', 'Next') + ' ›</span>';
  html += '<span style="font-size:11px;color:var(--text-3);margin-left:8px;">' + filtered.length + ' ' + tdT('td_records', 'records') + ' · ' + page + '/' + totalPages + '</span>';
...
function exportDetailCSV() {
  ...
  const headers = [tdT('td_time', 'Time'), tdT('th_model', 'Model'), tdT('th_status', 'Status'), 'Input', 'Output', 'CacheRead', 'CacheCreate', 'Total', tdT('th_duration', 'Duration'), tdT('th_client', 'Source'), tdT('th_path', 'Route'), tdT('th_error', 'Error')];
...
async function clearDetailHistory() {
  if (!confirm(tdT('td_clear_confirm', 'Clear all history? This cannot be undone.'))) return;
  const base = getBaseURL();
  const headers = {};
  if (window.LOCAL_AUTH_TOKEN) headers['X-Ocgt-Local-Token'] = window.LOCAL_AUTH_TOKEN;
