      ...
      <div className="set-card">
        <div className="set-row">
          ...
          <div className="control">
            <span className="tag green">{version}</span>
          </div>
        </div>
        <div className="set-row">
          <div className="label"><b>{t('sett_github')}</b><p>{t('sett_github_desc')}</p></div>
          <div className="control">
            <a className="btn btn-sm btn-ghost" href="https://github.com/ethan-blue/open-code-go-tools" target="_blank" rel="noopener">{t('btn_open_github')}</a>
          </div>
        </div>
        <div className="set-row">
        ...
        <div className="set-row">
          <div className="label"><b>{t('raw_json_title') || 'Edit Raw JSON'}</b><p>{t('raw_json_desc') || 'Directly edit the raw configuration file'}</p></div>
          <div className="control">
            ...
            }}>{t('raw_json_title') || 'Edit JSON'}</button>
          </div>
        </div>
      </div>
        ...
        <div className="modal-overlay on" onClick={(e) => { if (e.target === e.currentTarget) setRawJsonOpen(false) }}>
          <div className="modal-card modal-card-wide" role="dialog" aria-modal="true">
            ...
            <div className="modal-body">
              ...
              <textarea
                className="code-editor raw-json-editor settings-env-key"
                value={rawJsonContent}
                onChange={(e) => { setRawJsonContent(e.target.value); setRawJsonError('') }}
                spellCheck={false}
              />
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-sm btn-primary" disabled={rawJsonSaving} onClick={async () => {
                ...
                try {
                  ...
                  await apiFetch('/ocgt/api/config/raw', { method: 'POST', body: JSON.stringify({ content: rawJsonContent }) })
                  toast(t('raw_json_saved'), 'success'); setRawJsonOpen(false)
