...
export default function Copilot() {
  ...
  const loadInsights = async () => {
    ...
    try {
      ...
      toast(t('toast_insights_load_failed'), 'error')
      setInsights([])
  ...
  return (
    <div id="page-copilot">
      <div className="page-label"><span className="idx">★</span><span>{t('nav_copilot').toUpperCase()}</span><span className="path">/copilot</span><span className="copilot-action-badge">· {t('copilot_ai_badge')}</span></div>

      <div className="hero-wrap">
        <div className="eyebrow"><span className="pulse"></span> {t('copilot_eyebrow')}</div>
        <h1 className="hero">{t('copilot_hero')} <em>{t('copilot_hero_em')}</em></h1>
        <p className="lede copilot-lede">{t('copilot_desc')}</p>
