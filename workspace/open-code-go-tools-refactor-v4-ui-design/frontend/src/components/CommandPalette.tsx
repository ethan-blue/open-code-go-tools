...
export const CommandPalette: React.FC<CommandPaletteProps> = ({ isOpen, onClose }) => {
  ...
  const navigateItems = [
    { id: 'dashboard', label: t('cmd_goto_dashboard'), icon: '→', shortcut: '⌘1', action: () => navTo('dashboard') },
    { id: 'traffic', label: t('cmd_goto_traffic'), icon: '→', shortcut: '⌘2', action: () => navTo('history') },
    { id: 'sessions', label: t('cmd_goto_sessions'), icon: '→', shortcut: '⌘3', action: () => navTo('sessions') },
    { id: 'settings', label: t('cmd_open_settings'), icon: '→', shortcut: '⌘,', action: () => navTo('settings') },
    { id: 'providers', label: t('cmd_goto_providers'), icon: '→', shortcut: '⌘4', action: () => navTo('providers') },
    { id: 'hub', label: t('cmd_goto_hub'), icon: '→', shortcut: '⌘5', action: () => navTo('hub') },
    { id: 'quickconnect', label: t('cmd_quick_connect'), icon: '→', shortcut: '⌘6', action: () => navTo('terminal') },
    { id: 'copilot', label: t('cmd_copilot'), icon: '→', shortcut: '⌘7', action: () => navTo('copilot') }
  ];
  ...
  const actionItems = [
    ...
    { id: 'profile', label: t('cmd_switch_profile'), icon: '↻', shortcut: '⌘P', action: () => {
      window.dispatchEvent(new CustomEvent('nav-to', { detail: 'settings' }));
    } },
    { id: 'export', label: t('cmd_export_traffic'), icon: '⤓', shortcut: '⌘E', action: () => {
    ...
    { id: 'theme', label: t('cmd_toggle_theme'), icon: '◐', shortcut: '⌘T', action: () => {
