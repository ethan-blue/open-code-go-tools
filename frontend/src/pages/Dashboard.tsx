...
import DesignFootprint from '@/components/DesignFootprint'
 ...
 useEffect(() => {
  ...
  return (
    <div id="page-dashboard">
      <div className="page-label"><span className="idx">1</span><span>DASHBOARD</span><span className="path">/</span></div>

      <div className="hero-row">
        <div>
          <h1 className="hero">
            {status ? <>{t('dash_hero_ok')} <em>{t('dash_hero_ok_em')}</em></> : loading ? <>{t('dash_hero_loading')}</> : <>{t('dash_hero_off')} <em>{t('dash_hero_off_em')}</em></>}
          </h1>
          <p className="lede">
            {status ? (
              <>
                {t('dash_lede_ok_1')} <code className="inl">{status.listen}</code>, {t('dash_lede_ok_2')} {numClients} client{numClients !== 1 ? 's' : ''} {t('dash_lede_ok_3')} {status.upstream ? t('dash_lede_ok_up') : t('dash_lede_ok_local')} {t('dash_lede_ok_prov')}
                {summary && summary.total_requests > 0 && (<> {t('dash_lede_last_incident')} {errorRate < 0.1 ? t('dash_lede_na') : t('dash_lede_recently')}.</>)}
              </>
            ) : (<>{t('dash_lede_connecting')}</>)}
          </p>
        </div>
      ...
      <div className="panels">
        ...
        <div className="card">
          <div className="card-h">{t('dash_integrations')}<div className="actions"><button className="btn btn-sm btn-ghost" onClick={() => window.dispatchEvent(new CustomEvent('nav-to', { detail: 'quickconnect' }))}>{t('dash_manage')}</button></div></div>
                ...
                <div className="integ-row" key={row.key}>
                  ...
                  {row.active ? (
                    <>
                      ...
                      <button className="btn btn-sm btn-ghost" onClick={() => window.dispatchEvent(new CustomEvent('nav-to', { detail: 'quickconnect' }))}>{t('dash_configure')}</button>
                    </>
                  ) : (
                    <>
                      <span className="tag">{t('dash_inactive')}</span>
                      <button className="btn btn-sm btn-ghost" onClick={() => window.dispatchEvent(new CustomEvent('nav-to', { detail: 'quickconnect' }))}>{t('dash_install')}</button>
                    </>
                  )}
                </div>
              )
