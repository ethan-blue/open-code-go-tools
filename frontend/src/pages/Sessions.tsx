 ...
 useEffect(() => { localStorage.setItem('sessions_sort', sortBy) }, [sortBy])
  ...
  return (
    <div id="page-sessions">
      <div className="page-label"><span className="idx">5</span><span>{t('nav_sessions').toUpperCase()}</span><span className="path">/sessions</span></div>

      <div className="hero-row">
        <div>
          <h1 className="hero">{t('sessions_hero_title')}<em>{t('sessions_hero_title_em')}</em></h1>
          <p className="lede">{t('sessions_hero_desc')}</p>
        </div>
