...
import type { Provider, AgentLine, ProviderProtocol } from '@/lib/types'
...
function SortableProviderRow({ provider, onEdit, onDelete, onToggle }: { provider: Provider; onEdit: (p: Provider) => void; onDelete: (id: string) => void; onToggle: (id: string) => void }) {
  ...
  const healthConfig = { healthy: { icon: Zap, color: 'var(--online)', bg: 'var(--green-soft)', label: 'Healthy' }, degraded: { icon: AlertTriangle, color: 'var(--warn)', bg: 'rgba(245,158,11,0.1)', label: 'Degraded' }, down: { icon: Shield, color: 'var(--danger)', bg: 'rgba(239,68,68,0.1)', label: 'Down' }, unknown: { icon: Shield, color: 'var(--ink-400)', bg: 'var(--ink-100)', label: 'Unknown' } }
